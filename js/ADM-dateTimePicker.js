/*
 * @version 1.2.0 - modified by removing the jalali conversion
 *
...
(function (angular) {
    ...
    var ADMdtpFactory = function () {
        ...
        this.toRegularFormat = function (date, format) {
            if (!date)
                return false;

            date = typeof date == "string" ? this.parseString(date, format) : typeof date == "number" ? this.convertFromUnix(date) : date;
            return date ? [date.year, date.month.lZero(), date.day.lZero()].dtp_toDate() : false;
        };
        ...
        this.convertToUnix = function (value, format) {
            if (!value)
                return null;
            ...
            var _dateTime = new Date(this.toRegularFormat(value));
            return (/invalid/i.test(_dateTime)) ? null : _dateTime.getTime();
        };
        this.convertFromUnix = function (unix) {
        ...
        this.parseDisablePattern = function (options) {
            var arr = options.disabled, smart = options.smartDisabling, format = options.format;
                ...
                for (var i = 0, j = arr.length; i < j; i++) {
                    if (typeof arr[i] == "number") {
                        ...
                        if (arr[i].indexOf('d') == -1 && arr[i].indexOf('i') == -1) {
                            var _unix = this.convertToUnix(arr[i], format);
                            if (_unix)
                                _static[_unix] = true;
                        } else {
                            ...
                            if (_inMonthValid || _inWeekhValid) {
                                ...
                                if (_inMonthValid) {
                                    ...
                                    _patt.forEach(function (item) {
                                        var _params = item.split(/d[+]?/).map(function (str) {
                                            return Number(str);
                                        });
                                    ...
                                    var _tmpObj = {};
                                    _patt.forEach(function (item) {
                                        var _params = item.split(/i[+]?/).map(function (str) {
                                            return Number(str);
                                        });
                                        _params[1] %= 7;
                                        _tmpObj[_params[1]] = 1;
            ...
            return {smart: smart, static: _static, inWeek: _inWeek, inMonth: _inMonth};
        };

        this.isDayDisable = function (disabled, day) {
            if (disabled.static[day.unix])
                return true;

            var _dayName = (day.dayName + 7) % 7;
    ...
    var ADMdtpCalendarDirective = function (ADMdtp, ADMdtpFactory, constants, $timeout) {
        return {
            restrict: 'E',
            replace: true,
            //require: '^^admDtp',
            link: function (scope, element, attrs) {
                ...
                scope.modelChanged = function (input) {
                    ...
                    var _inputUnix = ADMdtpFactory.convertToUnix(_value, scope.option.format);
            ...
            template: '<div class="ADMdtp-box ADMdtp-calendar-container" ng-class="{square: monthPickerStat||timePickerStat}"> <div class="dtpNewBox" ng-class="{active: monthPickerStat}"> <i class="calendarIcon" ng-class="{show: monthPickerStat}" ng-click="monthPickerStat = false"> <svg class="dtp-i" viewBox="0 0 24 24"> <use xlink:href="#dtp-i-calendar" /> </svg> </i> <div class="content"> <div class="ADMdtpMonths" ng-class="{onYear: yearSelectStat}"> <div class="ADMdtpYears"> <svg class="dtp-i dtp-i-180 dtp-trs-3 arrow left" viewBox="0 0 24 24" ng-if="yearSelectStat" ng-click="previousYear()"> <use xlink:href="#dtp-i-right" /> </svg> <p class="dtp-trs-3" ng-click="selectYearInit()">{{current.year}}</p> <svg class="dtp-i dtp-trs-3 arrow right" viewBox="0 0 24 24" ng-if="yearSelectStat" ng-click="nextYear()"> <use xlink:href="#dtp-i-right" /> </svg> </div> <span ng-repeat="yearName in generatedYears" ng-if="yearSelectStat"><span class="dtp-trs-5" ng-class="{selected: yearName==current.year}" ng-click="selectYear(yearName)">{{yearName}}</span></span> <span ng-repeat="monthName in monthNames" ng-if="!yearSelectStat"><span class="dtp-trs-5" ng-class="{selected: monthName==current.monthDscr}" ng-click="selectMonth($index)">{{monthName}}</span></span> </div> </div> </div> <div class="dtpNewBox" ng-class="{active: timePickerStat}"> <i class="calendarIcon" ng-class="{show: timePickerStat}" ng-click="timePickerStat = false"> <svg class="dtp-i" viewBox="0 0 24 24"> <use xlink:href="#dtp-i-calendar" /> </svg> </i> <div class="content"> <div class="ADMdtpTime"> <span class="dtpIcon null up" ng-click="changeTimeValue(\'hour\', 1)"><svg class="dtp-i dtp-trs-5 dtp-i-270" viewBox="0 0 24 24"><use xlink:href="#dtp-i-right" /></svg></span><!-- --><span></span><!-- --><span class="dtpIcon null up" ng-click="changeTimeValue(\'minute\', 1)"><svg class="dtp-i dtp-trs-5 dtp-i-270" viewBox="0 0 24 24"><use xlink:href="#dtp-i-right" /></svg></span><!-- --><span>{{time.hour}}</span><!-- --><span class="period">:</span><!-- --><span>{{time.minute}}</span><!-- --><span class="dtpIcon null down" ng-click="changeTimeValue(\'hour\', -1)"><svg class="dtp-i dtp-trs-5 dtp-i-90" viewBox="0 0 24 24"><use xlink:href="#dtp-i-right" /></svg></span><!-- --><span></span><!-- --><span class="dtpIcon null down" ng-click="changeTimeValue(\'minute\', -1)"><svg class="dtp-i dtp-trs-5 dtp-i-90" viewBox="0 0 24 24"><use xlink:href="#dtp-i-right" /></svg></span> </div> </div> </div> <header> <svg class="dtp-i dtp-i-180 dtp-trs-3 arrow left" viewBox="0 0 24 24" ng-click="previousMonth()"> <use xlink:href="#dtp-i-right" /> </svg> <span class="yearMonth" ng-click="selectMonthInit()">{{current.monthDscr}} {{current.year}}</span> <svg class="dtp-i dtp-trs-3 arrow right" viewBox="0 0 24 24" ng-click="nextMonth()"> <use xlink:href="#dtp-i-right" /> </svg> </header> <div class="daysNames"> <span ng-repeat="dayName in daysNames">{{dayName}}</span> </div> <hr> <div class="ADMdtpDays" ng-class="{loading:loadingDays}"> <span ng-repeat="day in current.days" ng-click="selectThisDay(day)"><span ng-class="[{disable: day.disable||!day.valid, today: day.today, selected: day.selected, valid:(day.valid==2)}, (day.isMin)?\'min\':\'\', (day.isMax)?\'max\':\'\']">{{day.day}}</span></span> </div> <hr> <footer> <div class="calTypeContainer dtp-trs-3" ng-class="gregorian" ng-if="option.multiple"> <p class="gregorian">{{option.gregorianDic.title}}</p> </div> <button type="button" class="today dtp-trs-3" ng-click="today()">{{option["gregorianDic"].todayBtn}}</button> <svg class="dtp-i dtp-trs-5 timeSelectIcon ng-hide" viewBox="0 0 24 24" ng-show="option.dtpType != \'date\'" ng-click="timePickerStat = !timePickerStat"> <use xlink:href="#dtp-i-clock" /> </svg> </footer> </div>'
        };
    };

    var ADMdtpDirective = function (ADMdtp, ADMdtpFactory, constants, $compile, $timeout) {
        return {
            restrict: 'E',
            replace: true,
            transclude: true,
            ...
            link: function (scope, element, attrs, ctrls) {
                ...
                scope.monthNames = scope.option['gregorianDic'].monthsNames;
                scope.daysNames = scope.option['gregorianDic'].daysNames;
                ...
                scope.parseInputValue = function (valueStr, resetTime, releaseTheBeast) {
                    ...
                    if (valueStr) {
                        valueStr = ADMdtpFactory.toRegularFormat(valueStr, scope.option.format);
                ...
                if (scope.option.watchingOptions) {
                    ...
                    scope.$watch('options', function (nuVal, old) {
                        if (!nuVal || typeof nuVal != 'object')
                            return;
                        if (old && JSON.stringify(old) == JSON.stringify(nuVal))
                            return;
                ...
                attrs.$observe("mindate", function (_newVal) {
                    scope.$applyAsync(function () {
                        _newVal = scope.$eval(_newVal);
                        scope.minDate = ADMdtpFactory.convertToUnix(_newVal, scope.option.format);
                    });
                });
                ...
                attrs.$observe("maxdate", function (_newVal) {
                    scope.$applyAsync(function () {
                        _newVal = scope.$eval(_newVal);
                        scope.maxDate = ADMdtpFactory.convertToUnix(_newVal, scope.option.format);
                    });
                });
            ...
            controller: ['$scope', function ($scope) {
                    ...
                    this.fillDays = function (date, noTransition) {
                        ...
                        while (true) {
                            ...
                            if (ADMdtpFactory.isDayDisable($scope.disableDays, _day))
                                _day.valid = 0;
    ...
    return angular.module('ADM-dateTimePicker', [])
            ...
            .factory('ADMdtpFactory', [ADMdtpFactory])
            .directive('admDtp', ['ADMdtp', 'ADMdtpFactory', 'constants', '$compile', '$timeout', ADMdtpDirective])
            .directive('admDtpCalendar', ['ADMdtp', 'ADMdtpFactory', 'constants', '$timeout', ADMdtpCalendarDirective])
